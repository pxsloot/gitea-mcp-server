...
(``has_more=False``, ``next_offset=None``, known ``total_count``) via
``format.empty_paginated_result``, and ``tool_info``'s concise path reports
...
1. **FastMCP providers, not manual tool registration** -- The OpenAPI provider
   ...
   they expose the same agent-facing parameter, validation, formatting,
   and pagination contract through one shared spine.
