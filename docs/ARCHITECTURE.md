...
| `tools/virtual_params.py` | Virtual parameter registry + lifecycle (``inject_into``, ``extract_from``, ``apply_pre_hooks``, ``apply_to``) — generic mechanism for agent-facing params that are stripped before the HTTP call. Registered entries: ``sudo``, ``fetch_all``, ``content_type``, ``detail``, ``format``. Per-tool gating via ``tool_predicate`` (e.g. ``content_type`` only injects into tools with a ``content`` parameter). Output formatting via ``post_hook`` (``format`` applies ``apply_format`` using ``detail`` from the extracted dict). |
