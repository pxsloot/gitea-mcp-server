...
| `tools/synthetic_contract.py` | Shared validation and pagination schema contract for manually registered synthetic tools |
...
1. **FastMCP providers, not manual tool registration** -- The OpenAPI provider
   ...
   guides.  Their shared registration contract keeps pagination validation and
   output schemas aligned with generated tools without pretending they have an
   OpenAPI route.
   Synthetic and autogenerated tools may have different execution backends, but
   they should expose the same agent-facing parameter, validation, formatting,
