...
│       the _WRAP_ME marker (autogenerated tools today,   │
│       synthetic tools in a later stage); virtual-param  │
│       injection, validation, error handling, pagination │
│       metadata, formatting (registered server-level via │
│       mcp.add_transform, first in the chain)            │
...
provider — autogenerated tools today, identified by the ``_WRAP_ME`` marker
stamped during metadata customisation.  Synthetic tools still use their own
registration wrapper (``register_synthetic_tool``) and pass through the
transform unwrapped; stamping the marker and wiring them onto the shared
transform is the next stage of the generic transform refactor.
