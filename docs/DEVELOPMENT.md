...
2. **``_ToolWrappingTransform``** (registered **server-level** via
   ...
   contract spine (``tools/contract.build_transform_fn(tool, executor, default_format=...)``);
...
.. note::
    ...
    **Synthetic tools** (``search_tools``, ``search_resources``,
    ...
    page/limit slice in the result pipeline — no loop needed.  These tools
    are registered declaratively via
...
the result pipeline — no display logic lives in the registry.
...
pass it via :func:`inject_into`'s ``default_overrides`` parameter **and**
thread the same value into the contract spine as ``default_format`` so the
display path never falls back to a literal:
...
.. code-block:: python
    ...
    # In _ToolWrappingTransform._make_transform_fn()
    build_transform_fn(tool, executor, default_format=self._response_format)
...
- **Rule A (snake_case parameters)** — renames non-snake_case path/query/
  header/cookie parameters and body properties, converting both camelCase and
  kebab-case (``repository-id`` → ``repository_id``) to snake_case, recording
  the mapping in an ``x-param-rename`` extension on the operation.  The
  runtime shim (``mcp_builder._apply_param_rename``) corrects the
  ``parameter_map`` so the HTTP request still sends the original wire name —
  for path parameters it corrects ``openapi_name`` back to the original
  segment name, so the ``{placeholder}`` in the unchanged route path template
  is substituted with the original wire name; an agent that passes an old
  name gets the standard "Unknown parameter(s)" error.
...
3. **Registration closure** is a ``register_*`` function that takes ``mcp: FastMCP``
   ...
   are injected into the schema and popped before the executor runs.  The
   executor validates ``page``/``limit`` for paginated tools and calls the
   impl.  **The impl returns raw data only** — an
   :class:`~gitea_mcp_server.tools.result_pipeline.ExecutionResult` (data,
   total_count, shape) — which the single result pipeline
   (``tools/result_pipeline.render``) slices, envelopes, and formats.  No
   ``_formatted`` marker exists; display is centralized, not opt-out.
   ...
   pagination is one rule (``validate_pagination``) — both families name the
