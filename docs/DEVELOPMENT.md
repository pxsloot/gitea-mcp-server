...
| Impl return | Return raw data only — an ``ExecutionResult(data, total_count, shape)``. The single result pipeline slices (``list``), envelopes, and formats. For empty/out-of-range pages return ``shape="empty"`` with a ``message``; the pipeline emits the full envelope. For bespoke markdown (e.g. ``tool_info``, ``read_doc``) set ``markdown_formatter`` / ``markdown_extras`` on the result |
