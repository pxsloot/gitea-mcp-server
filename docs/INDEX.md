...
| Output format contract (json/raw vs markdown, `detail`) | `tools/virtual_params.py` (source, canonical: module docstring + `format`/`detail` descriptions) + `format.py` (renderer, points here) + `gitea_mcp_server/docs/guides/tool-output-format.md` (agent-facing echo) |
