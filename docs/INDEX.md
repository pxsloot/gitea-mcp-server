...
|-----|----------|------|--------|------------------|
| `gitea_mcp_server/docs/agent_instructions.md` | agent | reference + how-to | Tool/resource naming grammar, discovery flow, issue/PR workflow skeletons, output-format essentials, annotations, troubleshooting; points to the `output-format` guide for depth | You are an agent about to call a tool |
...
| Agent output contract (formats, envelopes, `$ref`, errors) | `gitea_mcp_server/docs/guides/output-format.md` (agent-facing reference) + `agent_instructions.md` (essentials + pointer) |
