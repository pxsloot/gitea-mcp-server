...
async def create_mcp_server(  # noqa: PLR0912, PLR0915 — server assembly inherently has many steps (spec loading, middleware, tool setup, resource setup, instructions)
    ...
    # provider) so it wraps tools from *all* providers — autogenerated tools
    # (stamped with the ``_WRAP_ME`` marker during metadata customisation)
    # and synthetic tools (stamped at registration by
