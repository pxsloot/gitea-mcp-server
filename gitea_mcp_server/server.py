...
def _setup_tool_discovery(  # noqa: PLR0913 - six params are acceptable for this orchestration function
    ...
    The contract wrapping transform (``_ToolWrappingTransform``) is already
    registered by the caller *before* this function runs, so it is the first
    server-level transform — tools it wraps then pass through the
    lazy-loading and namespace transforms here, matching the former
...
async def create_mcp_server(  # noqa: PLR0912, PLR0915 — server assembly inherently has many steps (spec loading, middleware, tool setup, resource setup, instructions)
    ...
    # Contract wrapping transform: registered server-level (not on the
    # provider) so it wraps tools from *all* providers — autogenerated
    # tools today (stamped with the ``_WRAP_ME`` marker), synthetic tools
    # in a later stage.  Added first so it runs before the lazy-loading,
    # namespace, and extension-metadata transforms, matching the previous
    # provider-level placement in ``list_tools`` order.
...
if __name__ == "__main__":
    main()  # pragma: no cover — can't be reached by pytest (confirmed #622)
