...
(:func:`_build_customization_meta`), and synthetic tools will stamp it at
...
class _ToolWrappingTransform(Transform):
    ...
    transform is the future home for both autogenerated and synthetic tools;
    today it wraps autogenerated tools, identified by the ``_WRAP_ME`` marker
    stamped in :func:`_build_customization_meta`.  Unmarked tools (including
    root-registered synthetic tools) pass through unchanged.
...
def create_openapi_provider(
    ...
    * ``provider.add_transform(…)`` -- label conversion (innermost).
    ...
    (autogenerated today, synthetic in a later stage).  No private
