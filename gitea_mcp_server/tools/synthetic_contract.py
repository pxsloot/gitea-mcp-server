...
:mod:`virtual_params` registry), validation, and post-hoc rendering all flow
through one shared spine (:func:`~tools.contract.build_transform_fn`).
...
- :func:`make_impl_executor` builds the per-tool executor the transform runs
  ...
  ``page``/``limit`` for paginated tools, and calls the impl.  The impl
  returns raw data — an
  :class:`~gitea_mcp_server.tools.result_pipeline.ExecutionResult` — which
  the single result pipeline renders.  No ``_formatted`` marker exists:
