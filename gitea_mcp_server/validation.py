...
def validate_pagination(
    ...
    *,
    ...
    The Gitea API spec names the page-size parameter ``per_page``
    (autogenerated tools); synthetic tools name it ``limit``.  The rule is
    identical; *page_size_name* controls the name reported in error messages
    and *page_size_max* the upper bound (synthetic tools may raise it via
    their per-tool ``limit_max``, e.g. ``read_doc`` allows 200).

    Args:
        page: Page number (integer >= 1), or ``None`` when absent.
        page_size: Items per page (integer between 1 and *page_size_max*),
            or ``None`` when absent.
        page_size_name: Public name of the page-size parameter used in
            error messages (``"limit"`` for synthetic tools, ``"per_page"``
            for the Gitea API spec).
