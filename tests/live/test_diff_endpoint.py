...
endpoint across three real transport layers:

1. **fastmcp.Client** over stdio (the primary agent-facing client)
2. **Raw MCP SDK** ``ClientSession`` over stdio
3. **Through the ``gitea_call_tool`` proxy** — exercises the proxy's own
   ``output_schema`` validation gate

All three scenarios require a real Gitea instance and real MCP server
...
# ---------------------------------------------------------------------------
# Scenario 3 — through the gitea_call_tool proxy
# ---------------------------------------------------------------------------


@live_available
class TestCallToolProxy:
    """Drive the diff endpoint through the ``gitea_call_tool`` synthetic proxy.

    The proxy has its own ``output_schema`` (``{"result": any}`` after the
    fix for #437).  This test verifies that the proxy does not reject the
    string diff result at the SDK validation gate.
    """
    ...
    async def test_through_call_tool_proxy_no_validation_error(
        ...
        """Call the diff tool *through* ``gitea_call_tool`` — full production path."""
        pr_number, owner, repo, _branch = live_test_data
        ...
        async with Client(transport) as client:
            result = await client.call_tool(
                "gitea_call_tool",
                {
                    "name": "gitea_repo_download_pull_diff_or_patch",
                    "arguments": {
                        "owner": owner,
                        "repo": repo,
                        "index": pr_number,
