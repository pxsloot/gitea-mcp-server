...
class TestAutoGeneratedSkipURIs:
    ...
    The skip URIs are now split between the factory (``_registered_uris``
    in ``resources/factory.py``) and the non-migrated custom resources
    (``_NON_FACTORY_SKIP_URIS`` in ``resources/auto.py``).  This test
    validates the combined set via ``_get_default_skip_uris()``.
    """

    def test_skip_uris_are_valid(self):
        from gitea_mcp_server.resources.auto import _get_default_skip_uris

        uris = _get_default_skip_uris()
        assert len(uris) >= 10
        # Most URIs have path-parameter templates; concrete URIs
        # like ``gitea://user`` are also valid (auto.py skips
        # paths without template params anyway).
        for uri in uris:
