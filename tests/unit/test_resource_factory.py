...
def _clearregistered_uris() -> None:
    """Clear ``registered_uris`` before each test to ensure test isolation.
...
class TestMakeApiResourceRegistration:
    ...
    def test_tracks_uri_inregistered_uris(self) -> None:
        mcp = _make_mock_mcp()
        client = _make_mock_client()
        spec = _make_mock_openapi_spec()
        ...
        assert handler is not None
        assert registered_uris == {"gitea://repos/{owner}/{repo}"}
