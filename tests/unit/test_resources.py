...
class TestRegisterAutoGeneratedResources:
    ...
    def test_registers_resources_for_get_endpoints(self, mock_gitea_client, openapi_spec_with_get):
        """Test that GET endpoints are registered as resources."""
        registry = ResourceRegistry()
        register_auto_generated_resources(
            registry, mock_gitea_client, openapi_spec_with_get, skip_uris=set()
        )
    ...
    def test_resource_function_has_docstring(self, mock_gitea_client, openapi_spec_with_get):
        """Test that auto-generated resources have proper docstrings."""
        registry = ResourceRegistry()
        register_auto_generated_resources(
            registry, mock_gitea_client, openapi_spec_with_get, skip_uris=set()
        )
        resource = registry.get("gitea://repos/{owner}/{repo}")
        assert resource is not None
        assert resource.func.__doc__ is not None
        assert "Get repository information" in resource.func.__doc__

    def test_ignores_non_get_methods(self, mock_gitea_client):
        ...
        registry = ResourceRegistry()
        register_auto_generated_resources(registry, mock_gitea_client, spec)
        resources = registry.list_resources()
        assert len(resources) == 0

    def test_handles_empty_spec(self, mock_gitea_client):
...
class TestRegisterCustomResources:
    ...
    def test_registers_all_custom_resources(self, mock_gitea_client):
        """Test that all expected custom resources are registered."""
        registry = ResourceRegistry()
        register_custom_resources(registry, mock_gitea_client)
        resources = registry.list_resources()
...
class TestAutoGeneratedResourceErrors:
    ...
    async def test_404_raises_structured_error(self, mock_gitea_client):
        ...
        registry = ResourceRegistry()
        register_auto_generated_resources(registry, mock_gitea_client, spec, skip_uris=set())

        resources = registry.list_resources()
        assert len(resources) == 1
        resource = resources[0]
        assert resource.uri == "gitea://test/{id}"
