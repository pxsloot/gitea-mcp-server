...
class TestDeriveRequiredScope:
    ...
    def test_put_delete_patch_are_write(self) -> None:
    ...
    def test_none_tags_returns_none(self) -> None:
    ...
    def test_missing_method_defaults_to_write(self) -> None:
        assert _derive_required_scope({"repository"}, None) == {"write:repository"}

    def test_unknown_tag_is_ignored(self) -> None:
        assert _derive_required_scope({"unknown", "repository"}, "GET") == {"read:repository"}

    def test_multi_tag_requires_all_scopes(self) -> None:
        """Multiple scope tags yield a conjunction, not a first-match pick.

        Mirrors the one multi-tag operation in the live spec:
        ``POST /user/repos`` (``createCurrentUserRepo``) carries
        ``["repository", "user"]`` and the router requires both categories.
        """
        assert _derive_required_scope({"repository", "user"}, "POST") == {
            "write:repository",
            "write:user",
        }
        assert _derive_required_scope({"repository", "user"}, "GET") == {
            "read:repository",
            "read:user",
        }

    def test_multi_tag_is_order_independent(self) -> None:
        """The result is a pure function of the tag set (hash-seed independent)."""
        forward = _derive_required_scope({"repository", "user", "issue"}, "POST")
        reverse = _derive_required_scope({"issue", "user", "repository"}, "POST")
        assert forward == reverse == {"write:repository", "write:user", "write:issue"}
