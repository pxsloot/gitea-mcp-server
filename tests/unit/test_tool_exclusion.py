...
class TestLoadExclusionConfig:
    ...
    def test_exclude_only(self, tmp_path: Path) -> None:
    ...
    def test_both_exclude_and_include(self, tmp_path: Path) -> None:
    ...
    def test_only_include_no_exclude(self, tmp_path: Path) -> None:
...
class TestPatternMatching:
    ...
    def test_prefix_matches_prefixed_name(self) -> None:
...
class TestSpecLevelExclusion:
    ...
    def test_exclude_by_exact_name(self, tmp_path: Path) -> None:
    ...
    def test_exclude_by_glob(self, tmp_path: Path) -> None:
        cfg = tmp_path / "cfg.yaml"
        cfg.write_text("exclude:\n  - admin_*")
        config = load_exclusion_config(str(cfg))
        filtered = compute_filtered_tools_info(
    ...
    def test_exclude_by_tag(self, tmp_path: Path) -> None:
        cfg = tmp_path / "cfg.yaml"
        cfg.write_text("exclude:\n  - tag:admin")
        config = load_exclusion_config(str(cfg))
        filtered = compute_filtered_tools_info(
    ...
    def test_exclude_star_with_include_whitelist(self, tmp_path: Path) -> None:
        cfg = tmp_path / "cfg.yaml"
        cfg.write_text('exclude:\n  - "*"\ninclude:\n  - admin_list_users')
        config = load_exclusion_config(str(cfg))
        filtered = compute_filtered_tools_info(
    ...
    def test_no_config_no_exclusion(self) -> None:
        filtered = compute_filtered_tools_info(
            SPEC,
